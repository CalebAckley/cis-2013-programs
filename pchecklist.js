...
// declares variables (bad passwords)
var passwordCheckOld = function () 
{
    var stringPassword, arrayBadPasswords, stringOutput;
    
    stringPassword = $("password").value; //pulls input from html
  ...
  /* sets conditions for password limitations
   */
  if (stringPassword.length < 8)
  {
    stringOutput = ("Password must be at least eight characters");
    $("password").value = "";
  }
  else
  {
    for (counter = 0; counter < arrayBadPasswords.length; counter++)
    {
        ...
        {
            stringOutput = ("Bad password, enter a new one");
            $("password").value = "";
            break;
        }
        else{
            stringOutput = ("Password accepted");
        }
    }
  }
